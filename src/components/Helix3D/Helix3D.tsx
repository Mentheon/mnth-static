...
/* =================================================================
   Mentheon — Helix3D concept (Rod of Asclepius + coiled serpent)
...
export default function Helix3D() {
  ...
  useEffect(() => {
    ...
    /* Rotation model: `rotorAngle` is the live Y rotation.
       `rotorTarget` is non-null only while easing toward a clicked
       node (snap). When neither hovered nor snapping, the rotor
       free-spins at ROTOR_FREE_SPEED rad/sec. */
    let rotorAngle = 0
    let rotorTarget: number | null = null
    const ROTOR_FREE_SPEED = 0.08
    ...
    /* Hover-on: mark a strand active. Recolours its orb crimson +
       emissive, flags the matching HTML label, pauses the free-spin
       (render() checks hoveredId), and shows the peek card. */
    ...
    /* Hover-off: reset every orb to its base colour, clear label
       flags, hide the peek, and let the free-spin resume. */
    function deactivate() {
    ...
    function render(dt: number, elapsed: number) {
      // 1. Rotor: ease to snap target if set; else free-spin unless
      //    a node is hovered (hover freezes rotation).
      if (rotorTarget !== null) {
